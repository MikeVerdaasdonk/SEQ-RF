...
df = pd.concat([df.iloc[:,0],spec_dummies,df.iloc[:,6:9], transport_dummies, df.iloc[:,11:12],df.iloc[:,14:15], df.iloc[:,19]], axis=1) #Dataset with new numerical columns(job and marital)
